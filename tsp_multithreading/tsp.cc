...
// The best cities permutation is returned.
Cities::permutation_t
tournament_search(const Cities &cities,
                  unsigned iters,
                  unsigned pop_size,
...
// using multithreading to compute the shortest distance with ga method
Cities::permutation_t
ga_search(const Cities &cities,
          ...
          unsigned nthread = 1)
{
  // initiate the ordering with 0, ..., cities.size()
  auto best_ordering = Cities::permutation_t(cities.size());
  for (unsigned int i = 0; i < cities.size(); i++)
  {
    best_ordering[i] = i;
  ...
  // a parameter to represent the thread we are running
  auto best_mutex = std::mutex();

  // a higher order function that perform ga_search
  auto run_one_thread = [&]()
  {
    // iterate (iters/nthread) times (because there are n threads, so the total number of iterations doesn't change)
    auto my_best = tournament_search(cities, iters / nthread, pop_size, mutation_rate);
    {
      // guard this thread using scoped_lock
      auto guard = std::scoped_lock(best_mutex);
      // Repeat check, maybe something changed:
      if (cities.total_path_distance(my_best) < cities.total_path_distance(best_ordering))
      {
        // update the best_ording if there's a better solution
        best_ordering = my_best;
      }
    }
  };

  // see how many threads are user using right now
  std::vector<std::thread> threads;

  // Every thread perform ga_search with iters/nthread
  for (unsigned i = 0; i < nthread; ++i)
  {
  ...
  // join the threads at last
  for (auto &t : threads)
  {
    t.join();
  }

  // return the best ordering
  return best_ordering;
}
...
int main(int argc, char **argv)
{
  ...
  cout << "best ordering: " << cities.total_path_distance(best_ordering) << endl;
